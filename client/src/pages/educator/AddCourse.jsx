...
const AddCourse = () => {
    ...
    setChapters(
      chapters.map((chapter)=>{
        if (chapter.chapterId === currentChapterId) {
          const newLecture = {
            ...lectureDetails,
            lectureOrder: chapter.chapterContent > 0 ? chapter.
  ...
  return (
    <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-4 pt-8 pb-0">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md w-full text-gray-500">
        <div className="flex flex-col gap-1">
          <p>Course Title</p>
          <input type="text" onChange={e=> setCourseTitle(e.target.value)} value={courseTitle} placeholder="Type here" 
          className="outline-none md:py-2.5 py-2 rounded border border-gray-500" required/>
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center justify-between flex-wrap">
          <div className="flex flex-col gap-1">
            <p>Course Details</p>
            <input type="number" onChange={e => setCoursePrice(e.target.value)} value={coursePrice} placeholder="0"
            ...
            showPopup && (
              <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
                <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                  ...
                  <div className="mb-2">
                    <p>Is Preview free?</p>
                    <input type="checkbox" className="mt-1 scale-125" value={lectureDetails.isPreviewFree}
                    onChange={(e)=>setLectureDetails({...lectureDetails,isPreviewFree: e.target.value})}/>
                  </div>
                  <button className="w-full bg-blue-400 text-white px-4 py-2 rounded" onClick={addLecture}>Add</button>
                  <img src={assets.cross_icon} alt="" className="absolute top-4 w-4 cursor-pointer" onClick={()=>handleChapter('remove', chapter.chapterId)}/>
                </div>
              </div>
            )
          }
